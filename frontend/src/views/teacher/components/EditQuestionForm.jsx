...
const AddQuestionForm = () => {
  ...
  const { data, refetch } = useGetQuestionByIdQuery({examId, questionId});
  useEffect(() => {
    refetch();
  }, [refetch]);
  ...
  useEffect(() => {
    if(data) {
      ...
      setMark(data.mark);
      setNegativeMark(data.negativeMark);
      if(data?.questionType === 'Multichoice') {
        setMultiChoiceOptions(data.multiOptions);
      };
      if(data?.questionType === 'TrueFalse') {
        setSelectedOneValue(data.selectedOneValue);
      };
      if(data?.questionType === 'SingleWord') {
        setSingleWordAnswers(data.singleWordAnswers);
      };
      if(data?.questionType === 'Essay') {
        setEssayAnswer(data.essay);
      };
    }
  ...
  const submitQuestionData = async (e) => {
    ...
    try {
      const res = await editQuestion(newQuestionObj).unwrap();
      if (res) {
